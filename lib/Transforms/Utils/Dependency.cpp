...
std::map<int, DependencyInstrInfoManager *> mgrs;
DependencyInstrInfoManager *llvm::getInfoManager(int index)
{
  return mgrs[index];
}
...
struct InterproceduralDependencyCheckPass : public FunctionPass {
  ...
  InterproceduralDependencyCheckPass() : FunctionPass(ID) {
  ...
  ~InterproceduralDependencyCheckPass() { delete dependency_map; }

  bool runOnFunction(Function &F) override {
    DependencyManager *dm =
        new DependencyManager(&F, dependency_map, annotated_map);
    dm->run();
    function_map[&F] = dm;
#if IDC_PRINT_RESULT
    print(&F);
